...
bool MainThread::StartNetSetting()
{
	...
	m_umListenSocket.insert(std::make_pair(NetLine::NetLine_GameS_User, hListenUser));
	CreateIoCompletionPort((HANDLE)m_umListenSocket[NetLine::NetLine_GameS_User], m_hIocp, (ULONG_PTR)m_umListenSocket[NetLine::NetLine_GameS_User], 0);
	if (PostAccept(NetLine::NetLine_GameS_User) == false)
	{
		return false;
	}
	return true;
