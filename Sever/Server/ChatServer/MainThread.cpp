...
{
	std::string strFilePath = "./Config/GameServerConfig.ini";
	bool bResult = false;
	char strTemp[256] = { 0, };
	int nTemp = 0;
	...
	m_nUserPort = GetPrivateProfileIntA("User", "PORT", 10479, strFilePath.c_str());
	return true;
}
...
bool MainThread::StartNetSetting()
{
	...
	addrUser.sin_addr.s_addr = ::htonl(INADDR_ANY);
	...
	m_umListenSocket.insert(std::make_pair(NetLine::NetLine_ChatS_User, hListenUser));

	CreateIoCompletionPort((HANDLE)m_umListenSocket[NetLine::NetLine_ChatS_User], m_hIocp, (ULONG_PTR)m_umListenSocket[NetLine::NetLine_ChatS_User], 0);

	if (PostAccept(NetLine::NetLine_ChatS_User) == false)
	{
		return false;
	}
	return true;
